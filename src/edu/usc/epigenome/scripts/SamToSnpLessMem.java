...
public class SamToSnpLessMem {
	...
	throws Exception
	{
		new SamToSnpLessMem().doMain(args);
	}
			...
			recordSub: while (chrItForAllele.hasNext())
			{
				...
				try
				{
					...
					if ((recCounter % 1E5)==0 && onRefCoord-10000 > 0)
					{
						...
						//System.err.printf("On new onRefCoord #%d\n",readsDepth.size());
						//Iterator<Integer> cleanKey = readsDepth.headMap(onRefCoord-10000).keySet().iterator();
						//while(cleanKey.hasNext()){
							//readsDepth.remove(cleanKey.next());
						//}
					}
