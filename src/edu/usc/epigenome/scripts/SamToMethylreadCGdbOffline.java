...
public class SamToMethylreadCGdbOffline {

	final private static String prefix = "methylReadCG_";
	final private static String USAGE = "SamToMethylreadCGdbOffline [opts] sampleName file1.bam file2.bam ...";
...
//	protected boolean outputReads = false;
	...
	throws Exception
	{
		new SamToMethylreadCGdbOffline().doMain(args);
	}
	...
	throws Exception {
		...
		for (final String chr : chrs)
		{
			...
			PrintWriter outWriter = Cpg.outputChromToFile(true, cytosines, prefix, sampleName, chr, this.minCphCoverage, this.minCphFrac);
			...
			for (final String fn : stringArgs)
			{
				...
				record: while (chrIt.hasNext())
				{
					if (canPurge && (lastBaseSeen > (lastPurge+PURGE_INTERVAL)))
					{
						...
						Cpg.outputCpgsToFile(true, outWriter, cytosines.headMap(new Integer(lastPurge)), prefix, sampleName, chr, this.minCphCoverage, this.minCphFrac);
...
//								}
								...
								if (iscpg || this.outputCphs)
								{
									...
									this.incrementCpg(cpg, seqi, i<convStart, nextBaseSeq, curReadId);
								}
...
//						System.exit(1);
			...
			if (cytosines != null) 	Cpg.outputCpgsToFile(true, outWriter, cytosines, prefix, sampleName, chr, this.minCphCoverage, this.minCphFrac);
			outWriter.close();
	...
	protected void incrementCpg(Cpg cpg, char seqChar, boolean nonconvFilter, char nextBaseSeq, int curReadId) 
	throws Exception
	{
		short totalReads = 0, cReads = 0, tReads = 0, cReadsNonconvFilt = 0, agReads = 0, nextBaseGreads = 0, nextBaseTotalReads = 0;
		...
		CpgRead cgread = new CpgRead(curReadId, cReads, cReadsNonconvFilt, tReads, agReads, nextBaseGreads, nextBaseSeq);
		cpg.addRead(cgread);
