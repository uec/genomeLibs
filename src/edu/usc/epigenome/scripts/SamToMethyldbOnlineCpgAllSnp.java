...
public class SamToMethyldbOnlineCpgAllSnp {
	 ...
	 */
		...
		final private static String USAGE = "SamToMethyldbOnlineCpgAllSnp [opts] sampleName snpfile2.txt file1.bam file2.bam ...";
...
//		protected boolean outputReads = false;
		...
		protected int minCpgBaseQual = 0;
		...
		throws Exception
		{
			new SamToMethyldbOnlineCpgAllSnp().doMain(args);
		}
...
//									}
									...
									if (conv && (this.useCpgsToFilter || (!ishcg & !isgcg)) && baseCpgQS > minCpgBaseQual) numConverted++;
									...
									else if( iscpg && baseCpgQS > minCpgBaseQual){
										if (seqFlag){
											...
											if (!allelePosition.subSet(readsStart, readsEnd).isEmpty()){									
												...
												List<String> cpgCreated = new ArrayList<String>();
												 while (allelePosIt.hasNext()){
													 ...
													 int pos = alleleChromPos - readsStart;
													 if(pos - i == 0){
									...
									if (iscpg)
									{
										if (i<convStart || baseCpgQS <= minCpgBaseQual)
										{
								...
								if (oppositeCpg)
								{
									...
									if (!allelePosition.subSet(0-readsEnd, 0-readsStart).isEmpty()){									
										Iterator<Integer> allelePosIt = allelePosition.subSet(0-readsEnd, 0-readsStart).iterator();
										//boolean noAllele = true;
										List<String> cpgCreated = new ArrayList<String>();
										 while (allelePosIt.hasNext()){
											 alleleChromPos = allelePosIt.next();
											 int pos = 0 - (alleleChromPos + readsStart);
											 if(pos - i == 0){
												 Iterator<String> cpgCreatedIt = cpgCreated.iterator();
												 while(cpgCreatedIt.hasNext()){
													 cytosines.remove(cpgCreatedIt.next());
