...
sub getCounts
{
	...
	my ($fh, $tmpout) = tempfile( sprintf("%s-%s.XXXXXX",substr($bambase,0,20),substr($bedbase,0,20)) , DIR => "/tmp");
	close($fh);

	my $cmd  = "$uecgatk -T CoverageDepth -R ${ref} -I ${bam} --intervals ${bed} -et NO_ET -o ${tmpout}.out > $tmpout";
	print STDERR "${cmd}...\n";

	`$cmd`;
	die "Can't open $tmpout\n" unless (open(F,$tmpout));
