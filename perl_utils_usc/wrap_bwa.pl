...

{
	...
	#bwa MEM
	my $cmd = join(" ", $bwa, "mem", "-M -t 20", $refFa, $read1, $read2, "> $outfileSAM");
	runcmd($cmd);
}
else
{
	...
	#aln end 1
	my $cmd = join(" ", $bwa, "aln", $phred, "-t 30", $refFa, $read1, "> $read1SA");
	runcmd($cmd);
	...
	if (@ARGV == 4)
	{
		...
		$cmd = join(" ", $bwa, "aln", $phred, "-t 30", $refFa, $read2, "> $read2SA");
		runcmd($cmd);
