...
#my $cmd .= "$JAVA -jar $PICARD REF_FLAT=$refFlat INPUT=$input OUTPUT=$output REFERENCE_SEQUENCE=$ref STRAND_SPECIFICITY=NONE METRIC_ACCUMULATION_LEVEL=ALL_READS TMP_DIR=$PICARDTMP";
my $cmd .= "$JAVA -jar $PICARD REF_FLAT=$refFlat INPUT=$input OUTPUT=$output REFERENCE_SEQUENCE=$ref STRAND_SPECIFICITY=NONE VALIDATION_STRINGENCY=SILENT TMP_DIR=$PICARDTMP";
runcmd($cmd);
